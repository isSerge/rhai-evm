...
A [Rhai](https://rhai.rs/) scripting engine plugin providing EVM token denomination helpers and primitive type conversions between `alloy-primitives` and Rhai `BigInt`.
...
2.  **Type Conversions**: Lossless conversion from `alloy-primitives` types (`U256`, `I256`) into Rhai-native `BigInt` values.
...
- **Includes BigInt**: Automatically integrates `rhai-bigint` so you have a complete environment.
...
Using the plugin requires registering both the `BigIntPackage` (which provides the math operations) and the `EvmPackage` (which provides the formatting).
